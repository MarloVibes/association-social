...
const CONSOLE_INFO: Record<string, { name: string; icon: string; color: string }> = {
  ps: { name: 'PlayStation', icon: '🎮', color: '#0070d1' },
  xbox: { name: 'Xbox', icon: '🟢', color: '#52b043' },
};
...
export default function LockerConsoleChatScreen() {
  const router = useRouter();
  const { console: consoleKey } = useLocalSearchParams<{ console: string }>();
  const info = CONSOLE_INFO[consoleKey] || { name: 'Chat', icon: '💬', color: '#22c55e' };
  ...
  useEffect(() => {
    ...
    // Cache user info for outgoing messages
    (async () => {
      try {
    ...
    const q = query(
      collection(db, 'locker_console_messages', consoleKey, 'messages'),
  ...
  async function handleSend(gifUrl?: string, imageUrl?: string) {
    ...
    if (!trimmed && !gifUrl && !imageUrl) return;
    const now = Date.now();
    ...
    try {
      await addDoc(collection(db, 'locker_console_messages', consoleKey, 'messages'), {
        text: trimmed,
        gifUrl: gifUrl || null,
  ...
  const setMsgReaction = async (msg: any, emoji: string) => {
    ...
    try {
      await updateDoc(doc(db, 'locker_console_messages', consoleKey, 'messages', msg.id), {
  ...
  const onMessageLongPress = (msg: any) => {
    ...
    if (isMine && editable) opts.push({ text: 'Delete', style: 'destructive', onPress: () => confirmDeleteMessage(msg) });
  ...
  const confirmDeleteMessage = (msg: any) => {
    Alert.alert('Delete message?', 'This permanently removes the message for everyone.', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try { await deleteDoc(doc(db, 'locker_console_messages', consoleKey, 'messages', msg.id)); }
        catch (e: any) { Alert.alert('Error', e.message); }
      } },
    ]);
  };
  ...
  const saveEdit = async () => {
    ...
    if (!trimmed) return;
    try {
      await updateDoc(doc(db, 'locker_console_messages', consoleKey, 'messages', editingMsg.id), {
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={0}>
      <View style={styles.header}>
        ...
        <View style={styles.titleWrap}>
          <Text style={styles.titleIcon}>{info.icon}</Text>
      ...
      {loading ? (
        <View style={styles.center}><ActivityIndicator color={info.color} /></View>
      ) : messages.length === 0 ? (
        <View style={styles.center}>
          <Text style={styles.emptyIcon}>💬</Text>
          <Text style={styles.emptyTitle}>No messages yet</Text>
          <Text style={styles.emptyDesc}>Be the first to say hi.</Text>
        </View>
      ) : (
        <FlatList
          ref={listRef}
      ...
      <Modal visible={showGiphy} animationType='slide' presentationStyle='pageSheet'>
        <View style={styles.giphyModal}>
          ...
          <TextInput
            ...
            autoFocus
          />
          {giphyLoading ? <ActivityIndicator color={info.color} style={{ marginTop: 20 }} /> : (
            <FlatList
              data={gifs}
              numColumns={2}
              keyExtractor={item => item.id}
