...
describe('source safety regressions', () => {
  ...
  it('runs commissioner season simulation continuously in fast batches', () => {
    ...
    expect(calendar).not.toContain('scrollToNextUnfinishedGame');
    expect(calendar).not.toContain('scrollToGameId');
    expect(calendar).not.toContain('seasonSimFollowGameId');
    expect(calendar).not.toContain('nextSimTargetAfter(');
    expect(calendar).not.toContain('simmedSeasonGameIdsRef');
    expect(calendar).not.toContain('CALENDAR_FOLLOW_VIEW_POSITION');
    expect(calendar).not.toContain('pendingFollowGameIdRef');
